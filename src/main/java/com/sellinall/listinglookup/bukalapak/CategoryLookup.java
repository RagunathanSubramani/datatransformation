...
public class CategoryLookup {
	...
	public static Object getCategorySpecifics(String countryCode, String categoryId, String accountNumber,
		...
		BasicDBObject bukalapakAttributesDB = getCategoryAttributesFromDB(countryCode, categoryId);
		if (bukalapakAttributesDB != null) {
			return bukalapakAttributesDB;
		}
		...
		JSONObject serviceResponse = HttpsURLConnectionUtil.doGet(Config.getConfig().getSiaBukalapakUrl() + "/category/"
	...
	private static BasicDBObject getCategoryAttributesFromDB(String countryCode, String categoryId) {
		BasicDBObject searchQuery1 = new BasicDBObject("countryCode", countryCode);
		BasicDBObject searchQuery2 = new BasicDBObject("categoryId", categoryId);
		BasicDBList and = new BasicDBList();
		and.add(searchQuery1);
		and.add(searchQuery2);

		BasicDBObject searchQuery = new BasicDBObject();
		searchQuery.put("$and", and);
		DBCollection table = DbUtilities.getLookupDBCollection("bukalapakAttributeLookup");
	...
	private static BasicDBObject persistToDB(String countryCode, String categoryId, JSONArray bukalapakAttributes)
		...
		updateData.put("attributes", JSON.parse(bukalapakAttributes.toString()));
