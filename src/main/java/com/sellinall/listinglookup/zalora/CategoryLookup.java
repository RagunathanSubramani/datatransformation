...
public class CategoryLookup {
	...
	public static Object getCategorySpecifics(String countryCode, String categoryId, String accountNumber,
		...
		BasicDBObject zaloraAttributesDB = getCategoryAttributesFromDB(countryCode, categoryId);
		if (zaloraAttributesDB != null) {
			return zaloraAttributesDB;
		}
		...
		JSONObject serviceResponse = HttpsURLConnectionUtil.doGet(Config.getConfig().getRocketEcomAdaptorUrl()
		...
		if (serviceResponse.getInt("httpCode") == HttpStatus.OK_200) {
			...
			if (response.has("SuccessResponse")) {
				...
				attributes = body.getJSONArray("Attribute");
			}
		}
	...
	private static BasicDBObject getCategoryAttributesFromDB(String countryCode, String categoryId) {
		...
		DBCollection table = DbUtilities.getLookupDBCollection("zaloraAttributeLookup");
	...
	private static BasicDBObject persistToDB(String countryCode, String categoryId, JSONArray zaloraAttributes)
		...
		updateData.put("attributes", JSON.parse(zaloraAttributes.toString()));
