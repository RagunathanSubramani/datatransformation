...
public class CategoryLookup {
	...
	private static JSONObject getCategorySpecificsFromEbay(String countryCode, String categoryId) {
		...
		if (GetCategorySpecificsResponse.has("Recommendations")) {
			...
			if (Recommendations.has("NameRecommendation")) {
				...
				if (Recommendations.get("NameRecommendation").getClass() == org.json.JSONArray.class) {
					...
					JSONObject NameRecommendationElement = new JSONObject(
							Recommendations.getJSONObject("NameRecommendation").toString());
			...
			log.error("Recommendations key not available on get category details from ebay for categoryID" + categoryId
					+ " countryCode:" + countryCode + " and reponse: " + GetCategorySpecificsResponse.toString());
			NameRecommendation = new JSONArray();
		}
